...
const WasteDisposal = () => {
    ...
    const handleCheckboxChange = (item) => {
        setSelectedItems((prev) => {
            const exists = prev.find((i) => i.itemNo === item.itemNo);

            if (exists) {
            // remove if already selected
            return prev.filter((i) => i.itemNo !== item.itemNo);
            } else {
            // add the whole row with quantity = 1
    ...
    const handleButtonClick = () => {
        console.log("Selected ItemNos:", selectedItems);
        setWasteModal(true);
    };
    ...
    const disposedItem = async () => {
        const itemNos = selectedItems.map(item => item.tagID);
        console.log(itemNos);
        try {
            const response = await axios.post(`${BASE_URL}/disposedItems.php`, {
                itemNos: itemNos   // 👈 send as { itemNos: [...] }
            });
            console.log('Scrapped Items:', response.data);
