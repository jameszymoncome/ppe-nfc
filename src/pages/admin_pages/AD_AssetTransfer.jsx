...
const handleAccept = async (transfer, file) => {
  ...
  const res = await fetch(`${BASE_URL}/em_getAssetTransfers.php?user_id=${user_id}`);
  const data = await res.json();
...
const fetchTransferItems = async (from_officerID) => {
  try {
    const res = await fetch(`${BASE_URL}/em_getAssetTransferItems.php?user_id=${from_officerID}`);
    const data = await res.json();
    return data.items || [];
  } catch (error) {
                ...
                {filteredData.length > 0 ? (
                  filteredData.map((item, index) => (
                    <tr
                      key={index}
                      onClick={() =>
                        navigate(`/assets/ad-asset-transfer-progress/${item.ptr_no}`, { state: { item } })
                      }
