...
const AssetTransfer3 = () => {
  ...
  let autoFillAssets = [];
  try {
    const response = await fetch(`${BASE_URL}/getAssignedAssets.php?user_id=${user_id}`);
    const data = await response.json();
...
const handleSubmit = async () => {
  try {
    ...
    const payload = {
      transferForm: {
        ...
        from_officer: user_id || transferForm.from_officer,
  ...
  useEffect(() => {
    ...
    const fetchAssets = async () => {
      try {
        const response = await fetch(`${BASE_URL}/getAssetsByUser.php?user_id=${user_id}`);
        const data = await response.json();
